...
def init_conf_args(_args, required_args=None, optional_args=None):
    ...
    _conf_args = {}
    if _args['conf_file']:
        _conf_args = ruamel.yaml.load(_args['conf_file'], Loader=ruamel.yaml.Loader)
    for r in required_args:
        _conf_args[r] = _args[r] if (r in _args and _args[r]) else _conf_args[r]
        try:
            assert _conf_args[r] is not None
        except AssertionError:
            print("[ERROR]", r, "not defined")
            raise
    for o in optional_args:
        _conf_args[o] = _args[o] if (o in _args and _args[o]) else (_conf_args[o] if o in _conf_args else None)
    _conf_args['user'] = _conf_args['user'] or os.environ['USER']
    _conf_args['user_duke_email'] = _conf_args['user_duke_email'] or "%s@duke.edu" % _conf_args['user']
    _conf_args['project_name'] = _conf_args['project_name'] or \
                                 os.path.splitext(os.path.basename(_args['metadata'].name))[0]
